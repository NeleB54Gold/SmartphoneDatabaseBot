...
class GSMArena
{
	...
	function getDevice ($device = '') {
		...
		if (!is_array($this->db) && $this->db->configs['redis']['status'] && $rr = $this->db->rget($url) && $r = json_decode($rr, 1)) {
		} else {
			$r = $this->requests->request($url, 0, 0, 1, 5);
			if (!is_array($this->db) && $this->db->configs['redis']['status']) $this->db->rset($url, json_encode($r), (60 * 60 * 2));
		}
